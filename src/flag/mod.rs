...
impl Flag {
    ...
    pub fn parse_value(&self, value: &str) -> Result<FlagValue, AppError> {
        ...
        match self.flag_type {
            ...
            FlagType::Integer => {
                let parsed: i64 = value.parse().map_err(|_| AppError::InvalidFlagValue {
            ...
            FlagType::Float => {
                let parsed: f64 = value.parse().map_err(|_| AppError::InvalidFlagValue {
                    flag: self.name.clone(),
                    value: value.to_string(),
            ...
            FlagType::IntegerList => {
                ...
                match parsed {
                    ...
                    Err(_) => Err(AppError::InvalidFlagValue {
                        flag: self.name.clone(),
                        value: value.to_string(),
    ...
    pub fn parse_values(&self, values: &[String]) -> Result<FlagValue, AppError> {
        match self.flag_type {
            FlagType::StringList => {
                for value in values {
                    self.validate_possible_values(value)?;
            ...
            _ => {
                if values.len() > 1 {
                    return Err(AppError::InvalidFlagValue {
                        flag: self.name.clone(),
                        value: values.join(", "),
    ...
    fn validate_possible_values(&self, value: &str) -> Result<(), AppError> {
